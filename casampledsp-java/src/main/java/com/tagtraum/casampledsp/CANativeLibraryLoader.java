...
public final class CANativeLibraryLoader {
  ...
  public static synchronized boolean loadLibrary() {
    ...
    try {
      ...
      Logger.getLogger(CANativeLibraryLoader.class.getName())
          .severe(
              "Failed to load native library 'casampledsp'. Please check your library path. CASampledSP will be dysfunctional.");
    }
   ...
   */
  public static File getClasspathOrJarDir(final Class<?> baseClass)
      throws UnsupportedEncodingException, MalformedURLException, FileNotFoundException {
    File directory = null;
    ...
    if (JAR_PROTOCOL.equals(url.getProtocol())) {
      ...
      directory =
          new File(
              path.substring(
                  0, path.length() - baseClass.getName().length() - CLASS_FILE_EXTENSION.length()));
    }
    return directory;
  }
    ...
    while (i < numChars) {
      ...
      if (c == '%') {
        ...
        try {
          ...
          while (((i + 2) < numChars) && (c == '%')) {
            ...
            if (v < 0) {
              throw new IllegalArgumentException(
                  "CANativeLibraryLoader: Illegal hex characters in escape (%) pattern - negative value");
            }
            bytes[pos++] = (byte) v;
            i += 3;
            if (i < numChars) {
          ...
          if ((i < numChars) && (c == '%')) {
            throw new IllegalArgumentException(
                "CANativeLibraryLoader: Incomplete trailing escape (%) pattern");
          }

          sb.append(new String(bytes, 0, pos, UTF_8));
        } catch (NumberFormatException e) {
          throw new IllegalArgumentException(
              "CANativeLibraryLoader: Illegal hex characters in escape (%) pattern - "
                  + e.getMessage());
        }
        needToChange = true;
      } else {
