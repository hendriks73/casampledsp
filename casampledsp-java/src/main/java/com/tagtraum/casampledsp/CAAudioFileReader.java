...
public class CAAudioFileReader extends AudioFileReader {
  ...
  private static void addAudioAudioFileFormatToCache(
  ...
  public AudioFileFormat getAudioFileFormat(final InputStream stream)
    ...
    if (!nativeLibraryLoaded)
      throw new UnsupportedAudioFileException("Native library casampledsp not loaded.");
    return getAudioFileFormat(stream, 0);
  }

  private AudioFileFormat getAudioFileFormat(final InputStream stream, final int fileTypeHint)
      throws IOException {
  ...
  public AudioFileFormat getAudioFileFormat(final URL url)
    ...
    if (isFile(url)) {
      audioFileFormat = intGetAudioFormat(url.toString().getBytes(StandardCharsets.UTF_8));
    } else {
  ...
  public AudioInputStream getAudioInputStream(final InputStream stream, final int bufferSize)
      throws UnsupportedAudioFileException, IOException {
    if (!nativeLibraryLoaded)
      throw new UnsupportedAudioFileException("Native library casampledsp not loaded.");
    final AudioFileFormat fileFormat = getAudioFileFormat(stream);
    return new CAAudioInputStream(
        new CAStreamInputStream(stream, 0, bufferSize),
  ...
  public AudioInputStream getAudioInputStream(final URL url)
    ...
    return getAudioInputStream(
        url,
        isFile(url)
            ? CAURLInputStream.getDefaultFileBufferSize()
            : CAStreamInputStream.getDefaultStreamBufferSize());
  }

  public AudioInputStream getAudioInputStream(final URL url, final int bufferSize)
      throws UnsupportedAudioFileException, IOException {
    if (!nativeLibraryLoaded)
      throw new UnsupportedAudioFileException("Native library casampledsp not loaded.");
  ...
  public AudioInputStream getAudioInputStream(final File file)
    ...
    if (!file.exists()) throw new FileNotFoundException(file.toString());
    if (!file.canRead()) throw new IOException("Can't read " + file);
    return getAudioInputStream(fileToURL(file), CAURLInputStream.getDefaultFileBufferSize());
  }

  public AudioInputStream getAudioInputStream(final File file, final int bufferSize)
      throws UnsupportedAudioFileException, IOException {
    if (!file.exists()) throw new FileNotFoundException(file.toString());
    if (!file.canRead()) throw new IOException("Can't read " + file);
  ...
  private native AudioFileFormat intGetAudioFormat(final byte[] urlBytes) throws IOException;

  private native AudioFileFormat intGetAudioFormat(
      final byte[] buf, final int length, final int fileTypeHint) throws IOException;
}
