...
public class TestCAURLInputStream {
  ...
  private int readThroughFile(final String prefix, final String filename)
    ...
    try (final CAAudioInputStream in =
        new CAAudioInputStream(
            new CAURLInputStream(file.toURI().toURL()),
  ...
  public void testNonExistingFile() throws IOException, UnsupportedAudioFileException {
    try (final CAURLInputStream in =
        new CAURLInputStream(new File("/Users/hendrik/bcisdbvigfeir.wav").toURI().toURL())) {
      in.read(new byte[1024]);
  ...
  public void testNonExistingURL() throws IOException, UnsupportedAudioFileException {
    try (final CAURLInputStream in =
        new CAURLInputStream(new URL("http://www.bubu.de/hendrik/bcisdbvigfeir.wav"))) {
      in.read(new byte[1024]);
      fail("Expected FileNotFoundException");
    } catch (FileNotFoundException e) {
      // expected this
