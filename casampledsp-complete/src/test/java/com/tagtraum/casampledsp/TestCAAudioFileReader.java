...
public class TestCAAudioFileReader {
  ...
  public void testGetAudioInputStreamURL() throws IOException, UnsupportedAudioFileException {
    ...
    try {
      final AudioInputStream audioInputStream =
          new CAAudioFileReader().getAudioInputStream(file.toURI().toURL());
  ...
  @Test
  public void testGetAudioInputStreamURLAndRead()
      throws IOException, UnsupportedAudioFileException {
    // first copy the file from resources to actual location in temp
    final String filename = "test.mp3";
    //        final File file = File.createTempFile("testGetAudioFileFormatURL", filename);
    final File file =
        new File(
            "/Users/hendrik/Music/iTunes/iTunes Music/Music/Bush/The Science of Things/11 Letting the Cables Sleep.mp3");
    extractFile(filename, file);
    int bytesRead = 0;
    try {
      final AudioInputStream in = new CAAudioFileReader().getAudioInputStream(file.toURI().toURL());
      AudioInputStream stream = in;
      final AudioFormat streamFormat = stream.getFormat();
      final int sampleSizeInBits =
          streamFormat.getSampleSizeInBits() > 0 ? streamFormat.getSampleSizeInBits() : 16;
      final int channels = streamFormat.getChannels() < 2 ? streamFormat.getChannels() : 2;
      final int frameSize =
          streamFormat.getFrameSize() > 0
              ? streamFormat.getFrameSize()
              : sampleSizeInBits * channels / 8;
      ...
      stream = new CAFormatConversionProvider().getAudioInputStream(targetFormat, stream);
      int justRead;
  ...
  @Test
  public void testGetAudioInputStreamURLAndSeek()
      throws IOException, UnsupportedAudioFileException {
    // first copy the file from resources to actual location in temp
    final String filename = "test.mp3";
